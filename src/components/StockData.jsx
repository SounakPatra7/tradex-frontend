...
useEffect(() => {
const fetchSymbolsAndStocks = async () => {
try {
const symbolRes = await axios.get('https://tradex-node.onrender.com/api/tiingo/stocks/symbols');
const extractedSymbols = symbolRes.data.stocks.map((stock) => stock.symbol);

if (extractedSymbols.length > 0) {
const stockRes = await axios.post('https://tradex-node.onrender.com/api/tiingo/stocks', {
...
console.error("Error fetching symbols or stocks:", err);
...
fetchSymbolsAndStocks();
}, []);

const filteredStocks = stocks.filter(
(stock) =>
stock?.symbol?.toUpperCase().includes(searchTerm) &&
stock?.name // Prevent entries without a name
);
